...
async function getTracks(ext) {
  ...
  // 使用正则提取 episodeList 数组的 JSON 字符串（具体正则需根据实际返回数据调整）
  const match = data.match(/"episodeList":\s*(\[[^\]]*\])/);
  if (match) {
    let episodeList = JSON.parse(match[1]);
