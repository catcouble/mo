...
async function test() {
    ...
    const searchResult = await search('西游记');
    ...
    if (searchData.list && searchData.list.length > 0) {
        ...
        const detailResult = await detail(firstItem.vod_id);
        ...
        if (detailData.list && detailData.list.length > 0) {
            ...
            if (vod.vod_play_url) {
                ...
                if (episodes.length > 0) {
                    ...
                    const playResult = await play(null, firstEpisode);
    ...
    const categoryResult = await category('1', '1');
